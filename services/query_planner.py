...
def _build_planner_prompt(
    ...
    return (
        ...
        "- 'earnings', 'kamai', 'billing', 'revenue', 'income' → the numeric payment/fees column "
        "AND, when no 'list'/'show'/'details' qualifier is present, set metric:sum (e.g. "
        "'earnings last quarter' → metric:sum, column:fees, time_range:last quarter).\n"
        "  Exception: if the user says 'show earnings' / 'list earnings' → metric:null (they want rows).\n"
        ...
        "- 'how many jobs', 'how many total jobs', 'total number of jobs', 'kitne kaam', 'kitne jobs' → metric:count, column:null, no filters (pure count of all records).\n"
        "- 'how many' + qualifier (e.g. 'how many invoices sent', 'how many paid') → metric:count + appropriate filter.\n"
        "- 'average fees', 'average billing', 'average per job', 'औसत' → metric:avg, column:fees.\n"
        "- 'biggest client', 'top client', 'largest client', 'top earner', 'biggest earner', 'highest earner', 'who earns me the most', 'top account', 'biggest account', 'sabse bada client', 'sabse zyada' → group_by:client_name, metric:sum, column:fees, order:desc, limit:1. An 'earner'/'account' is a CLIENT, not a single job — group by client. Keep any paid/date filter the user gave (e.g. 'top earner among paid jobs' → also filters:{paid:'yes'}).\n"
        "- 'highest paying job', 'most expensive job', 'biggest job', 'top earning job', 'which job paid the most' → metric:null, column:fees, order:desc, limit:1 (return the FULL job record ordered by fees, NOT by date). This is ONLY when the target word is a JOB/gig/project. Likewise 'lowest paying job'/'cheapest job' → column:fees, order:asc, limit:1. CRITICAL: you MUST set column:fees here, otherwise it sorts by date and returns the wrong job.\n"
        "- 'how much does [Client] owe me', '[Client] ka paisa', '[Client] se paisa aaya kya' → filters:{client_name:'[Client]', paid:'no'}, metric:sum, column:fees.\n"
        "- 'Star Studios se paisa aaya kya', 'Star Studios ka payment aaya' → this is asking if Star Studios paid; filters:{client_name:'Star Studios'}, metric:sum, column:fees, and also filter paid:'yes' (to see what WAS paid) OR filters:{client_name:'Star Studios', paid:'no'} (to see what's outstanding). When asking 'se paisa aaya kya' = 'has payment come from X?', default to metric:sum + paid:'yes' filter to show received amount.\n"
        "- 'how many invoices sent to clients with no email', 'clients without email' → filters:{bill_sent:'yes', poc_email:null}, metric:count (this is a compound filter).\n"
